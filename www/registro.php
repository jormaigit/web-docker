...
function restriccionesFormPhP($email,$contrasenia) {
  $emailRestriccion= "/^[^@]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/";
  ...
  &&
  !(preg_match($contraseniaLongRestriccion, $contrasenia) &&
    ...
    return "El correo electrónico y la contraseña no son validas";
  }
  if (!preg_match($emailRestriccion, $email)) {
...
<head>
    ...
    <script>
      // Para que el mensaje de error con "alert" desaparezca en X tiempo
      setTimeout(function() {
...
if (isset($_POST["submit"])) {
  ...
  // Si todos los campos requeridos están presentes...
  if(isset($_POST["nombre"]) && isset($_POST["correo"]) && isset($_POST["pass"]) && isset($_POST["passconfirm"]) &&
      !empty($_POST["nombre"]) && !empty($_POST["correo"]) && !empty($_POST["pass"]) && !empty($_POST["passconfirm"]) &&
      ...
      $validacion = restriccionesFormPhP($correo, $passw);
      ...
      if ($validacion === true) {
        ...
        $conexion->set_charset("utf8");
          ...
          if ($resultadocorreo->num_rows == 1) {
              echo "<div class='alert alert-danger'>El correo ya existe en la BD</div>";
              include("/html/registro.html");
          } else {
              // Hashea la contraseña antes de almacenarla en la base de datos
              $passwHashed = password_hash($passw, PASSWORD_DEFAULT);

              // Inserta el nuevo usuario en la base de datos
              $stmt = $conexion->prepare("INSERT INTO usuario (nombre, contrasenia, email) VALUES (?, ?, ?)");
              $stmt->bind_param("sss", $nombre, $passwHashed, $correo);
              $stmt->execute();
          ...
          echo "<div class='alert alert-danger'>$validacion</div>";
          include("html/registro.html");
      }
  } else {
      echo "<div class='alert alert-danger'>Todos los campos son obligatorios y las contraseñas deben coincidir</div>";
      include("html/registro.html");
  }
}
...
?>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark d-flex border-bottom border-2 pb-3 pt-3 border-light justify-content-center">
  <div class="navbar-collapse collapse w-100 order-1 order-lg-0">
    <ul class="navbar-nav mx-auto"></ul>
  </div>
  <div class="mx-auto text-center d-flex justify-content-center col-12 col-lg-12 order-0">
...
<!--FORMULARIO-->
<h4 class="text-light pt-5 text-center" >REGISTRARSE</h4>
<form class="container pt-5 pl-5 pr-5 pb-5  border-2 border-light" action="registro.php" method="post">

    <div class="form-outline mb-4">
      <label class="form-label text-light" for="form2Example1">Nombre: </label>  
      <input type="name" name="nombre" id="form2Example1" class="form-control" />    
      </div>

    <div class="form-outline mb-4">
    <label class="form-label text-light" for="form2Example1">Correo Electrónico: </label>  
    <input type="email" name="correo" id="form2Example1" class="form-control" />
    </div>

  
    <!-- INPUT DE LA CONTRASEÑA -->
    <div class="form-outline mb-4">
    <label class="form-label text-light" for="form2Example2">Contraseña: </label>  
    <input type="password" name="pass" id="form2Example2" class="form-control" />
    </div>
    
    <div class="form-outline mb-4">
    <label class="form-label text-light" for="form2Example2">Repetir contraseña: </label>    
    <input type="password" name="passconfirm" id="form2Example2" class="form-control" />
      </div>
    ...
    <!-- BOTÓN PARA ENVIAR -->
    <input type="submit" name="submit" value="Registrarme" class="btn btn-primary btn-block mb-4 text-light">
    <!-- BOTÓN DE REGISTRO -->
...
<!--PIE DE PÁGINA-->
<footer class="bg-dark text-white border-2 border-top border-white">
